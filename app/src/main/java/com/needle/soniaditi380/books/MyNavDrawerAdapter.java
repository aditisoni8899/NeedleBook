...
/**
 * Created by Aditya on 27-Jan-18.
 */
...
public class MyNavDrawerAdapter extends RecyclerView.Adapter<MyNavDrawerAdapter.ViewHolder> {
    ...
    private URL profile;        //int Resource for header view profile picture
    ...
    public MyNavDrawerAdapter(String Titles[], String Name, String Email, URL Profile, Context passedContext){ // MyNavDrawerAdapter Constructor with titles and icons parameter
    ...
    @Override
    public MyNavDrawerAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(MyNavDrawerAdapter.ViewHolder holder, int position) {
        if(holder.Holderid ==1) {                              // as the list view is going to be called after the header view so we decrement the
            // position by 1 and pass it to the holder while setting the text and image
            holder.textView.setText(mNavTitles[position - 1]); // Setting the Text with the array of our Titles
        }
        else{
            Picasso.with(holder.profile.getContext()).load(profile.toString()).into(holder.profile);// Similarly we set the resources for header view
